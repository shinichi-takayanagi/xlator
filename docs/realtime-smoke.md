...
5. Stop both while connecting and after connecting. Confirm microphone capture and translated playback stop immediately. A live session may receive final text for up to five seconds; populated rows remain visible and empty `処理中` rows disappear after this drain. Restart during the drain and confirm old events cannot modify the new log.
6. Open the browser developer console and inspect `window.__xlatorLatency`. Confirm that populated rows record `source-receipt-to-adoption`, `source-adoption-to-dom`, and equivalent `translation-*` stages. With local VAD timestamps available, also inspect `speech-to-source-receipt`, `speech-to-translation-receipt`, `speech-to-source-dom`, and `speech-to-translation-dom`. Check `silence-to-speech-end`, `silence-to-source-completed`, and `source-completed-to-dom` separately; DOM commit does not measure browser paint. Missing VAD timestamps must omit speech metrics. Compare these measurements with the direct Translation benchmark only after collecting repeated representative microphone runs.
